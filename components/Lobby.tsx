...
import { UserPlus, Play, Users, X, AlertCircle } from 'lucide-react';
...
const Lobby: React.FC<LobbyProps> = ({ players, onAddPlayer, onRemovePlayer, onStart }) => {
    ...
    <div className="w-full max-w-md mx-auto space-y-6 animate-slide-up">
      ...
      <Button 
        ...
        className={cn(
          ...
          !canStart ? "opacity-50 cursor-not-allowed bg-stone-300" : "bg-accent hover:bg-orange-600 text-white"
        )}
      >
      ...
      {!canStart && players.length > 0 && (
        <p className="text-center text-sm text-stone-500">Need at least 2 players to start.</p>
      )}
    </div>
  );
};
